...
`Pattern A` for displaying messages (Showing a single conversation). A list of messages that are a part of a message group are visible to users. For displaying message groups, use `Pattern B` (Showing a list of conversations). Users can check the other people they have been communicating with by viewing a list of messaging groups. For composing a fresh message in a fresh message group, use `Pattern C` (Create a message). For adding a new message to an existing message group, use `Pattern D` (update a message_group for the last message).
...
There are three types of items to insert in our Dynamo DB Table:
```
my_message_group = {
...
print(dict_users)
```
Note: Inside the file <bold>backend-flask/bin/db/setup</bold> we added the following code
